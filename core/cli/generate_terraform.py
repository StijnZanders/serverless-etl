...
class terraform:
    ...
    def _get_terraform_configuration_dags(self, dags):
        ...
        return {"resource": terraform_configuration}
    ...
    def _combine_configurations(self, configurations: []):
        ...
        for resources in configurations:

            for types in resources:

                if types not in terraform_configuration:
                    terraform_configuration[types] = {}

                for r in resources[types]:
                    for key in r:
                        terraform_configuration[types][key] = r[key]
